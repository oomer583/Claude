...
export function ToolsWorkspace() {
  ...
  const downloadLinks = useMemo(() => extractDownloadLinks(fileResult), [fileResult]);
  const editDownloadLinks = useMemo(() => extractDownloadLinks(editResult), [editResult]);
  ...
  const handleEditInstructionsChange = useCallback((event: ChangeEvent<HTMLTextAreaElement>) => {
    setEditInstructions(event.target.value);
  }, []);
  const handleEditFileChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-5 py-10 md:px-8">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-muted-foreground text-sm">
          Run isolated Python, create real Office/PDF files, or edit existing DOCX, XLSX, PPTX, and PDF files through Onyx Code Interpreter.
        </p>
      </div>
      ...
      <section className="grid gap-5 xl:grid-cols-3">
        <div className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm">
          ...
          <p className="mt-1 text-muted-foreground text-xs">Python runs in the configured isolated Code Interpreter service.</p>
          <textarea className="mt-4 min-h-64 w-full rounded-xl border border-border bg-background p-3 font-mono text-sm outline-none" onChange={handleCodeChange} spellCheck={false} value={code} />
          <Button className="mt-3" disabled={busy === "code" || !code.trim()} onClick={runCode}>
            {busy === "code" ? "Running..." : "Run Python"}
          </Button>
          {codeResult ? <pre className="mt-4 max-h-72 overflow-auto whitespace-pre-wrap rounded-xl bg-muted/40 p-3 text-xs">{codeResult}</pre> : null}
        </div>

        <div className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm">
          <h2 className="font-medium text-sm">File generation</h2>
          <p className="mt-1 text-muted-foreground text-xs">Create DOCX, XLSX, PPTX, or PDF output through the same sandbox.</p>
          <div className="mt-4 grid grid-cols-4 gap-2">
            {(["docx", "xlsx", "pptx", "pdf"] as const).map((value) => (
              <Button data-format={value} key={value} onClick={handleFormatClick} size="sm" variant={format === value ? "default" : "outline"}>
                {value.toUpperCase()}
              </Button>
            ))}
          </div>
          <input className="mt-3 h-9 w-full rounded-lg border border-border bg-background px-3 text-sm outline-none" onChange={handleFilenameChange} placeholder="Filename" value={filename} />
          <textarea className="mt-3 min-h-44 w-full rounded-xl border border-border bg-background p-3 text-sm outline-none" onChange={handleInstructionsChange} placeholder="Describe exactly what the file should contain..." value={instructions} />
          <Button className="mt-3" disabled={busy === "file" || !filename.trim() || !instructions.trim()} onClick={generateFile}>
              ...
              {downloadLinks.length > 0 ? (
                <div className="mt-3 flex flex-wrap gap-2 border-border/60 border-t pt-3">
                  {downloadLinks.map((link) => (
                    <Button asChild key={link.url} size="sm" variant="outline"><a href={link.url} rel="noopener noreferrer" target="_blank">{link.label}</a></Button>
                  ))}
                </div>
              ) : null}
            </div>
        ...
        <div className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm">
          ...
          <p className="mt-1 text-muted-foreground text-xs">Upload an existing file, describe the changes, and receive a new edited file.</p>
          <input accept=".docx,.xlsx,.pptx,.pdf" className="mt-4 block w-full text-sm" onChange={handleEditFileChange} type="file" />
          <textarea className="mt-3 min-h-44 w-full rounded-xl border border-border bg-background p-3 text-sm outline-none" onChange={handleEditInstructionsChange} placeholder="Example: Replace the title, keep the layout, and add a summary on the last page." value={editInstructions} />
          <Button className="mt-3" disabled={busy === "edit" || !editFile || !editInstructions.trim()} onClick={editOfficeFile}>
